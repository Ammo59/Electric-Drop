...
firstFrame = 154
frame1 = np.array([firstFrame, 273, 443, 640, 805, 920]) #805, 918, 1155, 1287, 1784, 1872 for 0kV
frame2 = np.array([93, 114, 237, 387, 635, 1118])#for 8kV interesting frames include: 93, 114, 237, 387, 635, 1118, 1227
time1 = (frame1 - firstFrame)/fps*1e3 # time elapsed (in ms) from the first frame (for frames to be displayed)
time2 = (frame2 - 93)/fps*1e3 # time elapsed (in ms) from the first frame (for frames to be displayed)
...
while(cap1.isOpened()): # loop through all the frames in the video (captures frame by frame)
    ...
    if frameCount == frame1[jj]: # check if this frame needs to be displayed
        ...
        ax.text(135, -20,'%3.1f ms' %time1[jj], color='black', fontsize=7, weight='bold') # display time associated with image
...
frameCount = 1 # frame counter initialization
jj = 6 # subplot counter initialization
...
while(cap2.isOpened()): # loop through all the frames in the video (captures frame by frame)
    ...
    if frameCount == frame2[jj]: # check if this frame needs to be displayed
        ...
        img = ax.imshow(frameImage) # display the image in the subplot
        img.set_cmap('gist_gray') # Set the color mode of the displayed image
        ax.text(135, -20,'%3.1f ms' %time2[jj], color='black', fontsize=7, weight='bold') # display time associated with image
        ax.set_axis_off() # these three commands hide the x,y axis in the plot
        ax.axes.get_xaxis().set_visible(False)
        ax.axes.get_yaxis().set_visible(False)
        jj=jj+1 # increment the counter
...
fig.savefig('Figure_4_mod2.png', bbox_inches='tight', pad_inches=0.0, dpi=300) #frameon=True
